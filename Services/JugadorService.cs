            ...
            try
            {
                using (var connection = _dbManager.GetConnection())
                {
                    ...
                    var command = new SqlCommand(
                        "INSERT INTO Jugadores (Nombre, Nivel) VALUES (@Nombre, @Nivel); SELECT SCOPE_IDENTITY();", connection);

                    // Asegúrate de que los parámetros están correctamente asignados
                    command.Parameters.AddWithValue("@Nombre", jugador.Nombre);
                    command.Parameters.AddWithValue("@Nivel", jugador.Nivel);

                    // Verifica que la consulta y la ejecución estén correctas
